...
{
    register_rest_route('kapitanpub/v1', '/track-click', array(
        'methods' => 'POST',
        ...
        'permission_callback' => function ($request) {
            ...
            if (!wp_verify_nonce($nonce, 'wp_rest')) {
                return new WP_Error('rest_nonce_invalid', __('Nonce is invalid.', 'kapitan-pub'), array('status' => 403));
            }
...
function kapitanpub_handle_track_click(WP_REST_Request $request)
{
    ...
    if (empty($link_type) || empty($target_url)) {
        return new WP_Error('missing_data', __('Missing link type or URL.', 'kapitan-pub'), array('status' => 400));
    }
    ...
    if (!in_array($link_type, $allowed_types)) {
        return new WP_Error('invalid_type', __('Invalid link type provided.', 'kapitan-pub'), array('status' => 400));
    }
    ...
    if ($result === false) {
        ...
        return new WP_Error('db_error', __('Could not save tracking data.', 'kapitan-pub'), array('status' => 500));
    }

    // Return success response
    return new WP_REST_Response(array('success' => true, 'message' => 'Click tracked successfully.'), 200);
}
